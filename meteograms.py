...
def get_datetime(fname):
   ...
   wrfout_d0?_2022-08-02_07:00:00
   """
   fname = fname.split('/')[-1]
...
dom = 'd02'
for dom in ['d02', 'd01']:
   com = f"ls {output_folder}/processed/wrfout_{dom}_{target_date.strftime('%Y-%m-%d')}_*"
   ...
   for iplace in range(len(Points)):
      ...
      folder = f"{plots_folder}/{dom}/{target_date.strftime('%Y/%m/%d')}"
